...
export default function EditPromotionPage() {
  ...
  const fetchPromotion = async () => {
    try {
      ...
      setForm({
        ...
        endTime: p.endTime || "",
        totalUsesLimit: p.totalUsesLimit?.toString() || "",
        perCustomerLimit: p.perCustomerLimit?.toString() || "",
        customerEligibility: p.customerEligibility || "ALL",
        eligibleTiers: p.eligibleTiers || [],
        showOnMenu: p.showOnMenu ?? true,
        status: p.status || "DRAFT",
        timesUsed: p.timesUsed || 0,
        totalDiscountGiven: p.totalDiscountGiven || 0,
      });
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.message || "Failed to update promotion");
    } finally {
      setSaving(false);
    }
  };
      ...
      <Card className="bg-muted/50">
        <CardContent className="py-4">
          <div className="flex items-center justify-around text-center">
            <div>
              <p className="text-2xl font-bold">{form.timesUsed}</p>
              <p className="text-sm text-muted-foreground">Times Used</p>
            </div>
            <div className="h-8 w-px bg-border" />
            <div>
              <p className="text-2xl font-bold">Rs.{form.totalDiscountGiven.toFixed(0)}</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {form.discountType === "PERCENTAGE" && (
              <div className="space-y-2">
                <label className="text-sm font-medium">Maximum Discount (Rs.)</label>
                <Input
                  type="number"
