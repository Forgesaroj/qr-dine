# QR DINE - Complete Session Flow Specification

## Document Info
- **Version**: 1.0
- **Created**: January 25, 2026
- **Status**: APPROVED
- **Related**: PROJECT-SPEC.md Section to be added

---

## Table of Contents
1. [Flow Overview](#1-flow-overview)
2. [QR Scan & Assistance Timer](#2-qr-scan--assistance-timer)
3. [Session Lifecycle](#3-session-lifecycle)
4. [Item-Level Time Tracking](#4-item-level-time-tracking)
5. [OTP Management](#5-otp-management)
6. [Table Cleaning Flow](#6-table-cleaning-flow)
7. [Table Duration & Alerts](#7-table-duration--alerts)
8. [Dashboard Views](#8-dashboard-views)
9. [Database Schema Updates](#9-database-schema-updates)
10. [API Endpoints](#10-api-endpoints)
11. [Notification Templates](#11-notification-templates)
12. [Settings & Configuration](#12-settings--configuration)

---

## 1. Flow Overview

### Complete Guest Session Timeline

```
═══════════════════════════════════════════════════════════════════════════════════
                         COMPLETE SESSION FLOW TIMELINE
═══════════════════════════════════════════════════════════════════════════════════

    GUEST ARRIVES              QR SCAN              OTP ENTRY            SESSION START
         │                        │                     │                      │
         ▼                        ▼                     ▼                      ▼
    ┌─────────┐              ┌─────────┐          ┌─────────┐           ┌─────────┐
    │ Sits at │              │ Scans   │          │ Enters  │           │ Session │
    │ Table 5 │─────────────▶│ QR Code │─────────▶│   OTP   │──────────▶│ Started │
    │         │              │         │          │         │           │         │
    └─────────┘              └────┬────┘          └────┬────┘           └────┬────┘
      2:55 PM                 2:56 PM              3:00 PM               3:00 PM
                                  │                    │                      │
                                  ▼                    │                      │
                         ┌───────────────┐            │                      │
                         │ ⏱️ 2-min Timer │            │                      │
                         │    Started    │            │                      │
                         └───────┬───────┘            │                      │
                                 │                    │                      │
                    ┌────────────┴────────────┐       │                      │
                    ▼                         ▼       │                      │
            [No OTP in 2 min]         [OTP Entered]   │                      │
                    │                         │       │                      │
                    ▼                         ▼       ▼                      │
           ┌───────────────┐          ┌─────────────────┐                   │
           │ 🔔 NOTIFY     │          │ ⏱️ 5-min Timer  │                   │
           │ "Needs Help"  │          │ (No order yet)  │                   │
           └───────────────┘          └────────┬────────┘                   │
                                               │                            │
                                  ┌────────────┴────────────┐               │
                                  ▼                         ▼               │
                          [No Order in 5 min]        [Order Placed]         │
                                  │                         │               │
                                  ▼                         │               │
                         ┌───────────────┐                  │               │
                         │ 🔔 NOTIFY     │                  │               │
                         │ "May need    │                  │               │
                         │  assistance" │                  │               │
                         └───────────────┘                  │               │
                                                           │               │
═══════════════════════════════════════════════════════════════════════════════════

      ORDER FLOW                 KITCHEN                  SERVICE              BILL
         │                          │                        │                   │
         ▼                          ▼                        ▼                   ▼
    ┌─────────┐              ┌─────────────┐          ┌─────────────┐     ┌─────────┐
    │ Guest   │              │   Kitchen   │          │   Waiter    │     │ Cashier │
    │ Orders  │─────────────▶│  Prepares   │─────────▶│   Serves    │────▶│  Bills  │
    │ (Cart)  │              │   Items     │          │   Items     │     │         │
    └────┬────┘              └──────┬──────┘          └──────┬──────┘     └────┬────┘
      3:02 PM                   3:05-3:25 PM            3:15-3:28 PM        4:02 PM
         │                          │                        │                   │
         │                          │                        │                   │
    ┌────┴────┐              ┌──────┴──────┐          ┌──────┴──────┐     ┌────┴────┐
    │ Track:  │              │ Track:      │          │ Track:      │     │ Shows:  │
    │ ordered │              │ preparing_at│          │ served_at   │     │ served  │
    │ _at     │              │ ready_at    │          │ served_by   │     │ times   │
    └─────────┘              └─────────────┘          └─────────────┘     └─────────┘

═══════════════════════════════════════════════════════════════════════════════════

     PAYMENT                SESSION END              CLEANING              READY
         │                      │                       │                    │
         ▼                      ▼                       ▼                    ▼
    ┌─────────┐           ┌───────────┐          ┌───────────┐        ┌─────────┐
    │ Payment │           │  Session  │          │  Waiter   │        │  Table  │
    │ Complete│──────────▶│   Ends    │─────────▶│  Cleans   │───────▶│Available│
    │         │           │           │          │           │        │         │
    └────┬────┘           └─────┬─────┘          └─────┬─────┘        └─────────┘
      4:05 PM               4:05 PM                4:08 PM             4:08 PM
         │                      │                       │
         │                      │                       │
    ┌────┴─────────────────────┴───────────────────────┴────┐
    │ AUTOMATIC ACTIONS:                                    │
    │ • OTP changed (new: 847)                             │
    │ • Table status → CLEANING                            │
    │ • Notify waiter for cleaning                         │
    │ • Voice messages deleted                             │
    │ • Session marked COMPLETED                           │
    └───────────────────────────────────────────────────────┘

═══════════════════════════════════════════════════════════════════════════════════
```

---

## 2. QR Scan & Assistance Timer

### 2.1 Flow Logic

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                        QR SCAN ASSISTANCE SYSTEM                                │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  TRIGGER: Guest scans QR code (page load)                                      │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  API Call: POST /api/guest/qr-scan                                      │   │
│  │  Body: { table_id, device_fingerprint, timestamp }                      │   │
│  │                                                                          │   │
│  │  Server Actions:                                                         │   │
│  │  1. Create QRScanEvent record                                           │   │
│  │  2. Start assistance timer (configurable, default: 2 min)               │   │
│  │  3. Return table info + OTP prompt                                      │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  CASE 1: Guest doesn't enter OTP within timer                                  │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  Background Job checks QRScanEvent records                              │   │
│  │  If: scan_time + timer_duration < now() AND otp_entered = false        │   │
│  │  Then: Send notification to waiter                                      │   │
│  │                                                                          │   │
│  │  Notification Type: GUEST_NEEDS_OTP_HELP                                │   │
│  │  Message: "Table {N} - Guest scanned QR but hasn't entered OTP"        │   │
│  │  Priority: MEDIUM                                                        │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                │
│  CASE 2: Guest enters OTP but doesn't order                                    │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  On successful OTP entry:                                                │   │
│  │  1. Mark QRScanEvent.otp_entered = true                                 │   │
│  │  2. Start new timer (configurable, default: 5 min)                      │   │
│  │                                                                          │   │
│  │  If: otp_time + order_timer < now() AND orders_count = 0               │   │
│  │  Then: Send notification to waiter                                      │   │
│  │                                                                          │   │
│  │  Notification Type: GUEST_BROWSING_NO_ORDER                             │   │
│  │  Message: "Table {N} - Guest browsing menu, may need assistance"       │   │
│  │  Priority: LOW                                                           │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  CASE 3: Guest completes order (Normal flow)                                   │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  On order placement:                                                     │   │
│  │  1. Cancel all pending assistance timers                                │   │
│  │  2. Mark QRScanEvent.order_placed = true                                │   │
│  │  3. Continue normal order flow                                          │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 2.2 Notification Messages (Case-Specific)

| Case | Notification Type | Message | Priority | Icon |
|------|------------------|---------|----------|------|
| No OTP entered | `GUEST_NEEDS_OTP_HELP` | "Table {N} - Guest scanned QR but hasn't entered OTP ({X} min ago)" | MEDIUM | 🆘 |
| OTP entered, no order | `GUEST_BROWSING_NO_ORDER` | "Table {N} - Guest browsing menu, may need assistance" | LOW | 👀 |
| Multiple failed OTP | `GUEST_OTP_FAILED` | "Table {N} - Guest entered wrong OTP {X} times" | HIGH | ⚠️ |

### 2.3 Database Schema for QR Scan Tracking

```prisma
model QRScanEvent {
  id                  String    @id @default(cuid())
  restaurant_id       String
  table_id            String

  // Tracking
  scanned_at          DateTime  @default(now())
  device_fingerprint  String?   // Browser fingerprint
  ip_address          String?
  user_agent          String?

  // Status flags
  otp_entered         Boolean   @default(false)
  otp_entered_at      DateTime?
  otp_attempts        Int       @default(0)

  order_placed        Boolean   @default(false)
  order_placed_at     DateTime?

  // Notifications sent
  otp_help_notified   Boolean   @default(false)
  otp_help_notified_at DateTime?

  browse_help_notified Boolean  @default(false)
  browse_help_notified_at DateTime?

  // Session created (if successful)
  session_id          String?

  // Relations
  restaurant          Restaurant @relation(fields: [restaurant_id], references: [id])
  table               Table      @relation(fields: [table_id], references: [id])
  session             TableSession? @relation(fields: [session_id], references: [id])

  @@index([restaurant_id, table_id, scanned_at])
  @@index([otp_entered, scanned_at])
}
```

---

## 3. Session Lifecycle

### 3.1 Session States

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                           SESSION STATE MACHINE                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│                              ┌─────────────┐                                   │
│                              │   CREATED   │                                   │
│                              │ (OTP Valid) │                                   │
│                              └──────┬──────┘                                   │
│                                     │                                          │
│                         ┌───────────┴───────────┐                              │
│                         ▼                       ▼                              │
│                  ┌─────────────┐         ┌─────────────┐                       │
│                  │   ACTIVE    │         │  ABANDONED  │                       │
│                  │(Guest Count)│         │ (No action  │                       │
│                  │   Entered   │         │  for 30min) │                       │
│                  └──────┬──────┘         └─────────────┘                       │
│                         │                                                      │
│                         ▼                                                      │
│                  ┌─────────────┐                                               │
│                  │  ORDERING   │◄────────────────┐                             │
│                  │ (Has items  │                 │                             │
│                  │  in cart)   │                 │                             │
│                  └──────┬──────┘                 │                             │
│                         │                        │                             │
│                         ▼                        │                             │
│                  ┌─────────────┐                 │                             │
│                  │   DINING    │─────────────────┘                             │
│                  │(Order being │  (Additional orders)                          │
│                  │  served)    │                                               │
│                  └──────┬──────┘                                               │
│                         │                                                      │
│                         ▼                                                      │
│                  ┌─────────────┐                                               │
│                  │    BILL     │                                               │
│                  │  REQUESTED  │                                               │
│                  └──────┬──────┘                                               │
│                         │                                                      │
│                         ▼                                                      │
│                  ┌─────────────┐                                               │
│                  │  COMPLETED  │                                               │
│                  │(Payment Done)│                                               │
│                  └─────────────┘                                               │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 3.2 Session Data Model

```prisma
model TableSession {
  id                  String          @id @default(cuid())
  restaurant_id       String
  table_id            String

  // Session Info
  session_number      Int             // Daily sequence: 1, 2, 3...
  status              SessionStatus   @default(CREATED)

  // Guest Info (STAFF ONLY CAN SET)
  guest_count         Int?
  guest_count_set_by  String?         // Staff ID
  guest_count_set_at  DateTime?

  // Timing
  created_at          DateTime        @default(now())
  started_at          DateTime?       // When guest count entered
  first_order_at      DateTime?       // First order placed
  last_order_at       DateTime?       // Last order placed
  bill_requested_at   DateTime?
  ended_at            DateTime?       // Payment completed

  // Billing
  bill_id             String?
  total_amount        Decimal?

  // OTP used for this session
  otp_used            String

  // Relations
  restaurant          Restaurant      @relation(...)
  table               Table           @relation(...)
  orders              Order[]
  voice_messages      VoiceMessage[]
  qr_scan_events      QRScanEvent[]
  guest_count_history GuestCountHistory[]

  @@index([restaurant_id, table_id, status])
  @@index([restaurant_id, created_at])
}

enum SessionStatus {
  CREATED           // OTP verified, waiting for guest count
  ACTIVE            // Guest count entered, session active
  ORDERING          // Has items in cart
  DINING            // Orders being served
  BILL_REQUESTED    // Guest requested bill
  COMPLETED         // Payment done, session ended
  ABANDONED         // No activity, auto-closed
}

// Track guest count changes (audit)
model GuestCountHistory {
  id            String        @id @default(cuid())
  session_id    String
  previous_count Int?
  new_count     Int
  changed_by    String        // Staff ID
  changed_at    DateTime      @default(now())
  reason        String?       // "Initial", "Guest added", "Guest left"

  session       TableSession  @relation(...)
  staff         User          @relation(...)
}
```

---

## 4. Item-Level Time Tracking

### 4.1 Order Item Timeline

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                         ORDER ITEM TIMELINE TRACKING                            │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ITEM: Chicken Momo (2x)                                                       │
│  ORDER: #ORD-0042                                                              │
│                                                                                 │
│  ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐      │
│  │ ORDERED │───▶│CONFIRMED│───▶│PREPARING│───▶│  READY  │───▶│ SERVED  │      │
│  └────┬────┘    └────┬────┘    └────┬────┘    └────┬────┘    └────┬────┘      │
│       │              │              │              │              │            │
│    3:02 PM        3:03 PM       3:05 PM        3:12 PM        3:15 PM         │
│       │              │              │              │              │            │
│       │              │              │              │              │            │
│  ┌────┴────┐    ┌────┴────┐    ┌────┴────┐    ┌────┴────┐    ┌────┴────┐      │
│  │ Guest   │    │ Waiter  │    │ Kitchen │    │ Kitchen │    │ Waiter  │      │
│  │ placed  │    │ confirms│    │ starts  │    │ marks   │    │ serves  │      │
│  │ order   │    │ order   │    │ cooking │    │ ready   │    │ to guest│      │
│  └─────────┘    └─────────┘    └─────────┘    └─────────┘    └─────────┘      │
│                                                                                 │
│  DURATIONS:                                                                    │
│  • Confirmation time: 1 min (3:02 → 3:03)                                     │
│  • Kitchen wait time: 2 min (3:03 → 3:05)                                     │
│  • Preparation time:  7 min (3:05 → 3:12)                                     │
│  • Pickup wait time:  3 min (3:12 → 3:15)                                     │
│  • TOTAL time:       13 min (3:02 → 3:15)                                     │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 4.2 Database Schema for Item Tracking

```prisma
model OrderItem {
  id                  String            @id @default(cuid())
  order_id            String
  menu_item_id        String

  // Item Details
  quantity            Int
  unit_price          Decimal           @db.Decimal(10, 2)
  total_price         Decimal           @db.Decimal(10, 2)
  notes               String?

  // Variant/Addon
  variant_id          String?
  addons              Json?             // [{addon_id, quantity, price}]

  // STATUS
  status              OrderItemStatus   @default(PENDING)

  // ═══════════════════════════════════════════════════════════════
  // TIME TRACKING (CRITICAL)
  // ═══════════════════════════════════════════════════════════════
  ordered_at          DateTime          @default(now())
  confirmed_at        DateTime?         // Waiter confirmed
  confirmed_by        String?           // Staff ID

  preparing_at        DateTime?         // Kitchen started
  preparing_by        String?           // Kitchen staff ID

  ready_at            DateTime?         // Kitchen completed
  ready_by            String?           // Kitchen staff ID

  served_at           DateTime?         // Waiter delivered to table
  served_by           String?           // Waiter ID

  cancelled_at        DateTime?
  cancelled_by        String?
  cancel_reason       String?
  // ═══════════════════════════════════════════════════════════════

  // Kitchen Station
  station             KitchenStation?   @default(HOT_KITCHEN)

  // Relations
  order               Order             @relation(...)
  menu_item           MenuItem          @relation(...)

  @@index([order_id, status])
  @@index([status, station])
}

enum OrderItemStatus {
  PENDING       // Just ordered, waiting confirmation
  CONFIRMED     // Waiter confirmed, sent to kitchen
  PREPARING     // Kitchen is cooking
  READY         // Ready for pickup
  SERVED        // Delivered to guest
  CANCELLED     // Cancelled
}

enum KitchenStation {
  HOT_KITCHEN   // Main cooking
  COLD_KITCHEN  // Salads, cold items
  GRILL         // Grilled items
  FRY           // Fried items
  BEVERAGES     // Drinks
  DESSERT       // Desserts
  BAR           // Alcoholic drinks
}
```

### 4.3 Waiter "Mark as Served" Flow

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                          WAITER SERVES ITEM FLOW                                │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  KITCHEN DISPLAY                        WAITER APP                              │
│  ┌─────────────────────────────┐       ┌─────────────────────────────┐         │
│  │  #ORD-0042 - Table 5        │       │  🔔 NOTIFICATION             │         │
│  │                             │       │                             │         │
│  │  2x Chicken Momo    [READY] │──────▶│  Order Ready for Pickup    │         │
│  │                             │       │  Table 5 - 2x Chicken Momo │         │
│  │  [ Mark All Picked Up ]     │       │                             │         │
│  └─────────────────────────────┘       │  [ Pick Up ]                │         │
│                                        └──────────────┬──────────────┘         │
│                                                       │                        │
│                                                       ▼                        │
│                                        ┌─────────────────────────────┐         │
│                                        │  WAITER PICKS UP ITEM       │         │
│                                        │  (Walks to table)           │         │
│                                        └──────────────┬──────────────┘         │
│                                                       │                        │
│                                                       ▼                        │
│                                        ┌─────────────────────────────┐         │
│                                        │  WAITER MARKS "SERVED"      │         │
│                                        │                             │         │
│                                        │  Table 5 - Items to Serve:  │         │
│                                        │  ☐ 2x Chicken Momo          │         │
│                                        │  ☐ 1x Coke                  │         │
│                                        │                             │         │
│                                        │  [ ✓ Mark Selected Served ] │         │
│                                        └──────────────┬──────────────┘         │
│                                                       │                        │
│                                                       ▼                        │
│                                        ┌─────────────────────────────┐         │
│                                        │  SYSTEM RECORDS:            │         │
│                                        │  • served_at = NOW()        │         │
│                                        │  • served_by = waiter_id    │         │
│                                        │  • status = SERVED          │         │
│                                        └─────────────────────────────┘         │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 4.4 Bill Display (Customer vs Internal)

**CUSTOMER BILL (Printed):**
```
┌─────────────────────────────────────────────────────────────┐
│                    QR DINE RESTAURANT                       │
│                   123 Main Street, KTM                      │
├─────────────────────────────────────────────────────────────┤
│  Bill No: #245                     Date: 2026-01-25        │
│  Table: 5                          Time: 4:02 PM           │
│  Guests: 4                         Server: Ram             │
├─────────────────────────────────────────────────────────────┤
│  Item                    Qty    Rate    Amount    Served   │
├─────────────────────────────────────────────────────────────┤
│  Chicken Momo             2    Rs.250   Rs.500    3:15 PM  │
│  Veg Fried Rice           1    Rs.350   Rs.350    3:20 PM  │
│  Coke                     1    Rs.100   Rs.100    3:05 PM  │
│  Chocolate Brownie        1    Rs.250   Rs.250    3:28 PM  │
├─────────────────────────────────────────────────────────────┤
│  Subtotal:                              Rs.1,200           │
│  Service Charge (10%):                  Rs.120             │
│  VAT (13%):                             Rs.172             │
├─────────────────────────────────────────────────────────────┤
│  GRAND TOTAL:                           Rs.1,492           │
├─────────────────────────────────────────────────────────────┤
│  Session: 3:00 PM - 4:02 PM (1h 2m)                       │
├─────────────────────────────────────────────────────────────┤
│                   Thank you for dining!                    │
└─────────────────────────────────────────────────────────────┘

NOTE: Only "Served" time shown on customer bill
```

**INTERNAL VIEW (Dashboard/Reports):**
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  ORDER #ORD-0042 - Table 5                                    INTERNAL VIEW    │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  Item              Qty   Ordered   Confirmed  Preparing   Ready    Served      │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Chicken Momo       2    3:02 PM   3:03 PM    3:05 PM    3:12 PM  3:15 PM     │
│                          (1 min)   (2 min)    (7 min)    (3 min)              │
│                                                                                 │
│  Veg Fried Rice     1    3:02 PM   3:03 PM    3:06 PM    3:18 PM  3:20 PM     │
│                          (1 min)   (3 min)    (12 min)   (2 min)              │
│                                                                                 │
│  Coke               1    3:02 PM   3:03 PM    3:03 PM    3:04 PM  3:05 PM     │
│                          (0 min)   (0 min)    (1 min)    (1 min)              │
│                                                                                 │
│  Chocolate Brownie  1    3:02 PM   3:03 PM    3:20 PM    3:25 PM  3:28 PM     │
│                          (1 min)   (17 min)   (5 min)    (3 min)              │
│                                                                                 │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  AVERAGES:                                                                      │
│  • Avg Confirmation: 0.75 min                                                  │
│  • Avg Kitchen Wait: 5.5 min                                                   │
│  • Avg Prep Time: 6.25 min                                                     │
│  • Avg Pickup Wait: 2.25 min                                                   │
│  • Avg Total: 13.5 min                                                         │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘

NOTE: Order number (#ORD-0042) shown internally, not on customer bill
```

---

## 5. OTP Management

### 5.1 OTP Lifecycle

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                              OTP LIFECYCLE                                      │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  TABLE CREATED                    SESSION ENDS                   NEW OTP       │
│       │                                │                            │          │
│       ▼                                ▼                            ▼          │
│  ┌─────────┐    SESSION 1         ┌─────────┐                  ┌─────────┐    │
│  │ OTP: 523│───────────────────▶ │ Payment │────────────────▶│ OTP: 847│    │
│  │ Active  │    (3:00-4:05 PM)   │ Complete│  Auto-generate  │ Active  │    │
│  └─────────┘                      └─────────┘                  └─────────┘    │
│                                                                                 │
│  WHY CHANGE OTP ON PAYMENT?                                                    │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  1. Previous guest can't scan again and access table                          │
│  2. Fresh start for next guest                                                 │
│  3. Security - prevents session hijacking                                      │
│  4. Clean audit trail per session                                             │
│                                                                                 │
│  OTP CHANGE TRIGGER:                                                           │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  ✅ Payment completed                                                          │
│  ✅ Session manually ended by staff                                            │
│  ✅ Session abandoned (auto-close after 30 min no activity)                   │
│  ❌ NOT on bill generation                                                     │
│  ❌ NOT on table cleaned                                                       │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 5.2 OTP History Tracking

```prisma
model OTPHistory {
  id              String    @id @default(cuid())
  restaurant_id   String
  table_id        String

  otp             String    // The OTP value

  // Lifecycle
  generated_at    DateTime  @default(now())
  generated_by    String?   // Staff ID or "SYSTEM"
  generation_reason String  // "TABLE_CREATED", "SESSION_ENDED", "MANUAL_RESET"

  used_at         DateTime? // When session started with this OTP
  session_id      String?   // Session that used this OTP

  expired_at      DateTime? // When replaced by new OTP
  expired_reason  String?   // "SESSION_ENDED", "MANUAL_RESET"

  // Relations
  restaurant      Restaurant @relation(...)
  table           Table      @relation(...)
  session         TableSession? @relation(...)

  @@index([restaurant_id, table_id, generated_at])
  @@index([otp, table_id])
}
```

### 5.3 OTP Change Implementation

```typescript
// services/otp.service.ts

export async function endSessionAndChangeOTP(sessionId: string) {
  return await prisma.$transaction(async (tx) => {

    // 1. Get session details
    const session = await tx.tableSession.findUnique({
      where: { id: sessionId },
      include: { table: true }
    });

    if (!session) throw new Error('Session not found');

    // 2. Mark current OTP as expired
    await tx.oTPHistory.updateMany({
      where: {
        table_id: session.table_id,
        expired_at: null
      },
      data: {
        expired_at: new Date(),
        expired_reason: 'SESSION_ENDED'
      }
    });

    // 3. Generate new OTP
    const newOTP = generateSecureOTP(); // 3-digit random

    // 4. Create new OTP history record
    await tx.oTPHistory.create({
      data: {
        restaurant_id: session.restaurant_id,
        table_id: session.table_id,
        otp: newOTP,
        generated_by: 'SYSTEM',
        generation_reason: 'SESSION_ENDED'
      }
    });

    // 5. Update table with new OTP
    await tx.table.update({
      where: { id: session.table_id },
      data: {
        current_otp: newOTP,
        otp_generated_at: new Date(),
        status: 'CLEANING'
      }
    });

    // 6. Mark session as completed
    await tx.tableSession.update({
      where: { id: sessionId },
      data: {
        status: 'COMPLETED',
        ended_at: new Date()
      }
    });

    // 7. Delete voice messages for this session
    const voiceMessages = await tx.voiceMessage.findMany({
      where: { session_id: sessionId }
    });

    // Delete audio files from storage
    for (const vm of voiceMessages) {
      await deleteAudioFile(vm.audio_url);
    }

    await tx.voiceMessage.deleteMany({
      where: { session_id: sessionId }
    });

    // 8. Send cleaning notification
    await sendNotification({
      type: 'TABLE_READY_FOR_CLEANING',
      restaurant_id: session.restaurant_id,
      table_id: session.table_id,
      data: {
        table_number: session.table.table_number,
        session_duration: calculateDuration(session.started_at, new Date()),
        guest_count: session.guest_count,
        total_amount: session.total_amount
      }
    });

    return { newOTP, tableId: session.table_id };
  });
}

function generateSecureOTP(): string {
  // Generate 3-digit OTP (100-999)
  return String(Math.floor(100 + Math.random() * 900));
}
```

---

## 6. Table Cleaning Flow

### 6.1 Cleaning Status Flow

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                           TABLE CLEANING FLOW                                   │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  PAYMENT COMPLETE                                                               │
│        │                                                                        │
│        ▼                                                                        │
│  ┌───────────────────────────────────────────────────────────────────────────┐ │
│  │  AUTOMATIC ACTIONS:                                                       │ │
│  │  1. Session status → COMPLETED                                           │ │
│  │  2. Table status → CLEANING                                              │ │
│  │  3. Generate new OTP                                                     │ │
│  │  4. Delete voice messages                                                │ │
│  │  5. Send notification to staff                                           │ │
│  └───────────────────────────────────────────────────────────────────────────┘ │
│        │                                                                        │
│        ▼                                                                        │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  🔔 NOTIFICATION TO STAFF                                                │   │
│  │                                                                          │   │
│  │  Recipients: Assigned waiter, All waiters, Host (configurable)          │   │
│  │                                                                          │   │
│  │  ┌────────────────────────────────────────────────────────────────────┐ │   │
│  │  │  🧹 Table 5 - Ready for Cleaning                                   │ │   │
│  │  │                                                                     │ │   │
│  │  │  Session: 3:00 PM - 4:05 PM (1h 5m)                               │ │   │
│  │  │  Guests: 4                                                         │ │   │
│  │  │  Bill: Rs.1,492                                                    │ │   │
│  │  │                                                                     │ │   │
│  │  │  [ Mark as Cleaned ]    [ Assign to Staff ]                       │ │   │
│  │  └────────────────────────────────────────────────────────────────────┘ │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│        │                                                                        │
│        │         ┌───────────────────────────────────────────────────────┐     │
│        │         │  ⏱️ CLEANING TIME LIMIT ALERT (Optional)             │     │
│        │         │                                                       │     │
│        │         │  If table not cleaned within X minutes:              │     │
│        │         │  • Show warning in dashboard                         │     │
│        │         │  • Escalate to manager (configurable)                │     │
│        │         │                                                       │     │
│        │         │  Example: "Table 5 waiting for cleaning (15 min)" ⚠️ │     │
│        │         └───────────────────────────────────────────────────────┘     │
│        │                                                                        │
│        ▼                                                                        │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  STAFF MARKS TABLE CLEANED                                              │   │
│  │                                                                          │   │
│  │  Who can mark: Any staff (Waiter, Host, Manager, Cashier)              │   │
│  │                                                                          │   │
│  │  ┌────────────────────────────────────────────────────────────────────┐ │   │
│  │  │  Mark Table 5 as Cleaned?                                          │ │   │
│  │  │                                                                     │ │   │
│  │  │  Cleaning checklist (optional):                                    │ │   │
│  │  │  ☑ Table wiped                                                     │ │   │
│  │  │  ☑ Chairs arranged                                                 │ │   │
│  │  │  ☑ Floor checked                                                   │ │   │
│  │  │  ☑ Condiments restocked                                            │ │   │
│  │  │                                                                     │ │   │
│  │  │  [ Cancel ]    [ ✓ Mark as Cleaned ]                              │ │   │
│  │  └────────────────────────────────────────────────────────────────────┘ │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│        │                                                                        │
│        ▼                                                                        │
│  ┌───────────────────────────────────────────────────────────────────────────┐ │
│  │  TABLE STATUS → AVAILABLE                                                 │ │
│  │  Ready for next guest!                                                   │ │
│  └───────────────────────────────────────────────────────────────────────────┘ │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 6.2 Cleaning Record Schema

```prisma
model TableCleaningRecord {
  id              String    @id @default(cuid())
  restaurant_id   String
  table_id        String
  session_id      String?   // Previous session

  // Timing
  cleaning_requested_at DateTime @default(now())
  cleaned_at            DateTime?
  cleaning_duration     Int?      // Minutes taken

  // Staff
  cleaned_by      String?   // Staff ID

  // Checklist (optional)
  checklist       Json?     // {table_wiped: true, chairs: true, ...}

  // Alert tracking
  alert_sent_at   DateTime?
  escalated_at    DateTime?

  // Relations
  restaurant      Restaurant @relation(...)
  table           Table      @relation(...)
  session         TableSession? @relation(...)
  staff           User?      @relation(...)

  @@index([restaurant_id, cleaning_requested_at])
}
```

---

## 7. Table Duration & Alerts

### 7.1 Duration Display Rules

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                        TABLE DURATION DISPLAY RULES                             │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  DURATION CALCULATION:                                                         │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Start time = session.started_at (when guest count entered)                    │
│  Current duration = NOW() - session.started_at                                 │
│                                                                                 │
│  COLOR CODING (Configurable):                                                  │
│  ─────────────────────────────────────────────────────────────────────────────  │
│                                                                                 │
│  Duration        Color       Meaning                                           │
│  ───────────────────────────────────────────────────────────────────────────── │
│  0-30 min        🟢 Green    Normal dining time                                │
│  30-60 min       🟡 Yellow   Getting longer                                    │
│  60-90 min       🟠 Orange   Long stay                                         │
│  90+ min         🔴 Red      Very long stay                                    │
│                                                                                 │
│  TABLE CARD EXAMPLES:                                                          │
│  ─────────────────────────────────────────────────────────────────────────────  │
│                                                                                 │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐           │
│  │  Table 1    │  │  Table 5    │  │  Table 8    │  │  Table 12   │           │
│  │  🟡 OCCUPIED│  │  🟡 OCCUPIED│  │  🟡 OCCUPIED│  │  🟡 OCCUPIED│           │
│  │  👥 4       │  │  👥 2       │  │  👥 6       │  │  👥 3       │           │
│  │  ⏱️ 15m 🟢  │  │  ⏱️ 45m 🟡  │  │  ⏱️ 1h 10m🟠│  │  ⏱️ 2h 🔴   │           │
│  └─────────────┘  └─────────────┘  └─────────────┘  └─────────────┘           │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 7.2 Long Stay Alert System

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                          LONG STAY ALERT SYSTEM                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ALERT CONFIGURATION (Settings → Table Management → Alerts):                   │
│  ─────────────────────────────────────────────────────────────────────────────  │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  Long Stay Alerts                                          [Enabled ✓] │   │
│  │                                                                         │   │
│  │  Send notification after table occupied for: [ 90 ] minutes            │   │
│  │                                                                         │   │
│  │  Notify:                                                                │   │
│  │  ☑ Assigned waiter                                                     │   │
│  │  ☑ Manager                                                             │   │
│  │  ☐ All waiters                                                         │   │
│  │                                                                         │   │
│  │  Repeat notification every: [ 30 ] minutes                             │   │
│  │                                                                         │   │
│  │  Message: "Table {N} occupied for {duration} - Check if OK"            │   │
│  │                                                                         │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  ALERT FLOW:                                                                   │
│  ─────────────────────────────────────────────────────────────────────────────  │
│                                                                                 │
│  Session starts at 3:00 PM                                                     │
│        │                                                                        │
│        ├── 3:30 PM: Duration 30m → Yellow color                                │
│        │                                                                        │
│        ├── 4:00 PM: Duration 60m → Orange color                                │
│        │                                                                        │
│        ├── 4:30 PM: Duration 90m → Red color + 🔔 NOTIFICATION                 │
│        │            "Table 5 occupied for 1h 30m - Check if assistance needed" │
│        │                                                                        │
│        ├── 5:00 PM: Duration 120m → 🔔 REPEAT NOTIFICATION                     │
│        │            "Table 5 still occupied (2h) - Please check"               │
│        │                                                                        │
│        └── ... continues every 30 min                                          │
│                                                                                 │
│  WAITER CAN:                                                                   │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  • Dismiss alert (with reason: "Guest confirmed staying")                      │
│  • Snooze alert (30 min)                                                       │
│  • Request bill on behalf of guest                                             │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

## 8. Dashboard Views

### 8.1 Waiter Dashboard (Standard View)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  👤 Ram (Waiter)                              🔔 3     ⚙️    [Logout]          │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐                           │
│  │ My      │  │ Pending │  │ Voice   │  │ Need    │                           │
│  │ Tables  │  │ Orders  │  │ Messages│  │ Assist  │                           │
│  │   8     │  │   2     │  │   1     │  │   1     │                           │
│  └─────────┘  └─────────┘  └─────────┘  └─────────┘                           │
│                                                                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│  [All] [My Tables] [Needs Attention]        🔍 Search    [Grid ▣] [List ☰]    │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐   │
│  │  Table 1      │  │  Table 2      │  │  Table 3      │  │  Table 4      │   │
│  │  🟢 AVAILABLE │  │  🟡 OCCUPIED  │  │  🟡 OCCUPIED  │  │  🔴 BILL REQ  │   │
│  │               │  │  👥 4         │  │  👥 2         │  │  👥 6         │   │
│  │               │  │  ⏱️ 25m 🟢    │  │  ⏱️ 1h 5m 🟠  │  │  ⏱️ 45m 🟡    │   │
│  │               │  │  📋 2 orders  │  │  📋 1 order   │  │  💰 Rs.3.2K   │   │
│  └───────────────┘  └───────────────┘  └───────────────┘  └───────────────┘   │
│                                                                                 │
│  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐   │
│  │  Table 5      │  │  Table 6      │  │  Table 7      │  │  Table 8      │   │
│  │  🟣 CLEANING  │  │  🟢 AVAILABLE │  │  🟠 ATTENTION │  │  🟡 OCCUPIED  │   │
│  │               │  │               │  │  👥 3         │  │  👥 2         │   │
│  │  🧹 5m ago    │  │               │  │  ⏱️ 35m 🟡    │  │  ⏱️ 10m 🟢    │   │
│  │               │  │               │  │  🔔 Help req  │  │  📋 1 order   │   │
│  └───────────────┘  └───────────────┘  └───────────────┘  └───────────────┘   │
│                                                                                 │
│  WAITER TABLE CARD INFO (STANDARD):                                            │
│  • Table number                                                                │
│  • Status (color coded)                                                        │
│  • Guest count (if occupied)                                                   │
│  • Duration (with color coding)                                                │
│  • Order count OR bill amount (if bill requested)                             │
│  • Alert indicator (if needs attention)                                        │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 8.2 Manager Dashboard (Detailed View)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  👔 Krishna (Manager)                         🔔 5     ⚙️    [Logout]          │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐              │
│  │ Today's │  │ Active  │  │ Pending │  │ Avg     │  │ Alerts  │              │
│  │ Sales   │  │ Tables  │  │ Orders  │  │ Time    │  │         │              │
│  │ Rs.45K  │  │  12/20  │  │   3     │  │  42m    │  │   2     │              │
│  └─────────┘  └─────────┘  └─────────┘  └─────────┘  └─────────┘              │
│                                                                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│  FLOOR OVERVIEW                                           [Map View] [List]    │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ┌───────────────────────────────────────────────────────────────────────────┐ │
│  │  Table 2 (Detailed)                                          [Expand ↗]  │ │
│  │  ─────────────────────────────────────────────────────────────────────── │ │
│  │  Status: 🟡 OCCUPIED          Waiter: Ram                                │ │
│  │  Guests: 4                    Duration: 25m 🟢                           │ │
│  │                                                                           │ │
│  │  ORDERS:                                                                  │ │
│  │  #ORD-0042 • 3:02 PM • 5 items • Rs.1,200 • Status: SERVING             │ │
│  │    └─ 2/5 items served                                                   │ │
│  │  #ORD-0045 • 3:18 PM • 2 items • Rs.350 • Status: PREPARING             │ │
│  │    └─ 0/2 items served                                                   │ │
│  │                                                                           │ │
│  │  TIMELINE:                                                                │ │
│  │  3:00 PM - Session started (Guest count: 4)                              │ │
│  │  3:02 PM - Order #ORD-0042 placed                                        │ │
│  │  3:05 PM - Order confirmed                                               │ │
│  │  3:15 PM - 2 items served                                                │ │
│  │  3:18 PM - Additional order #ORD-0045                                    │ │
│  │                                                                           │ │
│  │  Est. Bill: Rs.1,550 (before tax)                                        │ │
│  │                                                                           │ │
│  │  [ View Full Details ]  [ Chat with Waiter ]  [ Request Bill ]           │ │
│  └───────────────────────────────────────────────────────────────────────────┘ │
│                                                                                 │
│  MANAGER TABLE CARD INFO (DETAILED):                                           │
│  • Everything in Standard view PLUS:                                           │
│  • Assigned waiter name                                                        │
│  • All orders with status                                                      │
│  • Items served count                                                          │
│  • Session timeline                                                            │
│  • Estimated bill amount                                                       │
│  • Quick actions (view details, chat, request bill)                           │
│                                                                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│  ⚠️ ALERTS                                                                     │
├─────────────────────────────────────────────────────────────────────────────────┤
│  🔴 Table 12 - Occupied 2h+ (Check on guests)              [ Dismiss ] [View] │
│  🟡 Table 5 - Waiting for cleaning (10 min)                [ Dismiss ] [View] │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

## 9. Database Schema Updates

### 9.1 Complete Schema Additions

```prisma
// ═══════════════════════════════════════════════════════════════════════════════
// TABLE SCHEMA UPDATE
// ═══════════════════════════════════════════════════════════════════════════════

model Table {
  id                  String        @id @default(cuid())
  restaurant_id       String

  // Basic Info
  table_number        String
  name                String?
  capacity            Int
  floor               String?       @default("Ground")
  section             String?

  // QR & OTP
  qr_code             String?       @unique
  current_otp         String
  otp_generated_at    DateTime      @default(now())

  // Position (for floor plan)
  position_x          Int?
  position_y          Int?

  // Status
  status              TableStatus   @default(AVAILABLE)

  // Timestamps
  created_at          DateTime      @default(now())
  updated_at          DateTime      @updatedAt

  // Relations
  restaurant          Restaurant    @relation(...)
  sessions            TableSession[]
  otp_history         OTPHistory[]
  qr_scan_events      QRScanEvent[]
  cleaning_records    TableCleaningRecord[]

  @@unique([restaurant_id, table_number])
  @@index([restaurant_id, status])
}

enum TableStatus {
  AVAILABLE         // Ready for guests
  OCCUPIED          // Has active session
  RESERVED          // Reserved for future
  BILL_REQUESTED    // Guest requested bill
  CLEANING          // Being cleaned
  BLOCKED           // Temporarily unavailable
  NEEDS_ATTENTION   // Guest requested help
}


// ═══════════════════════════════════════════════════════════════════════════════
// SETTINGS SCHEMA
// ═══════════════════════════════════════════════════════════════════════════════

model RestaurantSettings {
  id                  String    @id @default(cuid())
  restaurant_id       String    @unique

  // ─────────────────────────────────────────────────────────────────────────────
  // GUEST ASSISTANCE SETTINGS
  // ─────────────────────────────────────────────────────────────────────────────
  assistance_enabled          Boolean   @default(true)
  otp_help_timer_minutes      Int       @default(2)     // Notify if no OTP in X min
  order_help_timer_minutes    Int       @default(5)     // Notify if no order in X min

  // ─────────────────────────────────────────────────────────────────────────────
  // TABLE DURATION SETTINGS
  // ─────────────────────────────────────────────────────────────────────────────
  duration_green_max          Int       @default(30)    // Minutes
  duration_yellow_max         Int       @default(60)    // Minutes
  duration_orange_max         Int       @default(90)    // Minutes
  // After orange_max = Red

  // ─────────────────────────────────────────────────────────────────────────────
  // LONG STAY ALERTS
  // ─────────────────────────────────────────────────────────────────────────────
  long_stay_alert_enabled     Boolean   @default(true)
  long_stay_alert_minutes     Int       @default(90)    // First alert after X min
  long_stay_repeat_minutes    Int       @default(30)    // Repeat every X min
  long_stay_notify_waiter     Boolean   @default(true)
  long_stay_notify_manager    Boolean   @default(true)

  // ─────────────────────────────────────────────────────────────────────────────
  // CLEANING SETTINGS
  // ─────────────────────────────────────────────────────────────────────────────
  cleaning_alert_enabled      Boolean   @default(true)
  cleaning_alert_minutes      Int       @default(10)    // Alert if not cleaned in X min
  cleaning_checklist_enabled  Boolean   @default(false)
  cleaning_checklist_items    Json?     // ["Table wiped", "Chairs arranged", ...]

  // Relations
  restaurant        Restaurant  @relation(...)
}
```

---

## 10. API Endpoints

### 10.1 QR Scan & Assistance APIs

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                           QR SCAN & ASSISTANCE APIs                             │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  POST /api/guest/qr-scan                                                       │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Triggered when guest scans QR code (page load)                                │
│                                                                                 │
│  Request:                                                                       │
│  {                                                                              │
│    "table_id": "tbl_xxx",                                                      │
│    "device_fingerprint": "abc123",                                             │
│    "user_agent": "Mozilla/5.0..."                                              │
│  }                                                                              │
│                                                                                 │
│  Response:                                                                      │
│  {                                                                              │
│    "table": { "number": "5", "name": "Window Seat" },                         │
│    "restaurant": { "name": "QR Dine Restaurant", "logo": "..." },             │
│    "requires_otp": true,                                                       │
│    "scan_event_id": "scan_xxx"                                                 │
│  }                                                                              │
│                                                                                 │
│  POST /api/guest/verify-otp                                                    │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Verify OTP and start/join session                                             │
│                                                                                 │
│  Request:                                                                       │
│  {                                                                              │
│    "table_id": "tbl_xxx",                                                      │
│    "otp": "523",                                                               │
│    "scan_event_id": "scan_xxx"                                                 │
│  }                                                                              │
│                                                                                 │
│  Response (Success):                                                            │
│  {                                                                              │
│    "success": true,                                                            │
│    "session": {                                                                │
│      "id": "sess_xxx",                                                         │
│      "status": "CREATED",                                                      │
│      "guest_count": null  // Not set yet                                       │
│    },                                                                          │
│    "guest_token": "jwt_token_xxx"                                              │
│  }                                                                              │
│                                                                                 │
│  Response (Wrong OTP):                                                          │
│  {                                                                              │
│    "success": false,                                                           │
│    "error": "INVALID_OTP",                                                     │
│    "attempts": 2,                                                              │
│    "max_attempts": 5                                                           │
│  }                                                                              │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 10.2 Session APIs

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                              SESSION APIs                                       │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  POST /api/sessions/:id/guest-count  (STAFF ONLY)                              │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Set/update guest count for session                                            │
│                                                                                 │
│  Request:                                                                       │
│  {                                                                              │
│    "guest_count": 4,                                                           │
│    "reason": "Initial"  // or "Guest added", "Guest left"                      │
│  }                                                                              │
│                                                                                 │
│  POST /api/sessions/:id/end  (STAFF ONLY)                                      │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  End session after payment                                                      │
│                                                                                 │
│  Actions performed:                                                             │
│  - Session status → COMPLETED                                                  │
│  - Table status → CLEANING                                                     │
│  - Generate new OTP                                                            │
│  - Delete voice messages                                                       │
│  - Send cleaning notification                                                  │
│                                                                                 │
│  GET /api/sessions/:id/timeline                                                │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Get complete session timeline                                                  │
│                                                                                 │
│  Response:                                                                      │
│  {                                                                              │
│    "session": { ... },                                                         │
│    "timeline": [                                                               │
│      { "time": "3:00 PM", "event": "Session started", "by": "System" },       │
│      { "time": "3:00 PM", "event": "Guest count: 4", "by": "Ram (Waiter)" },  │
│      { "time": "3:02 PM", "event": "Order #ORD-0042 placed", "by": "Guest" }, │
│      { "time": "3:03 PM", "event": "Order confirmed", "by": "Ram" },          │
│      ...                                                                       │
│    ]                                                                           │
│  }                                                                              │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 10.3 Order Item Status APIs

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                          ORDER ITEM STATUS APIs                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  PATCH /api/orders/:orderId/items/:itemId/status                               │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Update item status with timestamp                                              │
│                                                                                 │
│  Request:                                                                       │
│  {                                                                              │
│    "status": "PREPARING"  // or "READY", "SERVED"                              │
│  }                                                                              │
│                                                                                 │
│  System automatically sets:                                                     │
│  - preparing_at / ready_at / served_at = NOW()                                 │
│  - preparing_by / ready_by / served_by = current_staff_id                      │
│                                                                                 │
│  POST /api/orders/:orderId/items/bulk-serve  (WAITER)                          │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Mark multiple items as served at once                                          │
│                                                                                 │
│  Request:                                                                       │
│  {                                                                              │
│    "item_ids": ["item_1", "item_2", "item_3"]                                  │
│  }                                                                              │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 10.4 Table Cleaning APIs

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                          TABLE CLEANING APIs                                    │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  POST /api/tables/:id/mark-cleaned  (ANY STAFF)                                │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Mark table as cleaned and available                                            │
│                                                                                 │
│  Request:                                                                       │
│  {                                                                              │
│    "checklist": {          // Optional, if enabled                             │
│      "table_wiped": true,                                                      │
│      "chairs_arranged": true,                                                  │
│      "floor_checked": true,                                                    │
│      "condiments_restocked": true                                              │
│    }                                                                           │
│  }                                                                              │
│                                                                                 │
│  Actions performed:                                                             │
│  - Table status → AVAILABLE                                                    │
│  - Create TableCleaningRecord                                                  │
│  - Cancel any cleaning alerts                                                  │
│                                                                                 │
│  GET /api/tables/cleaning-queue                                                │
│  ─────────────────────────────────────────────────────────────────────────────  │
│  Get all tables waiting for cleaning                                            │
│                                                                                 │
│  Response:                                                                      │
│  {                                                                              │
│    "tables": [                                                                 │
│      {                                                                         │
│        "id": "tbl_xxx",                                                        │
│        "table_number": "5",                                                    │
│        "cleaning_requested_at": "2026-01-25T16:05:00Z",                       │
│        "waiting_minutes": 8,                                                   │
│        "previous_session": {                                                   │
│          "guest_count": 4,                                                     │
│          "duration_minutes": 65,                                               │
│          "total_amount": 1492                                                  │
│        }                                                                       │
│      }                                                                         │
│    ]                                                                           │
│  }                                                                              │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

## 11. Notification Templates

### 11.1 All Notification Types

| Type | Recipients | Message Template | Priority |
|------|------------|------------------|----------|
| `GUEST_NEEDS_OTP_HELP` | Assigned waiter, All waiters | "Table {N} - Guest scanned QR but hasn't entered OTP ({X} min ago)" | MEDIUM |
| `GUEST_BROWSING_NO_ORDER` | Assigned waiter | "Table {N} - Guest browsing menu, may need assistance" | LOW |
| `GUEST_OTP_FAILED` | Assigned waiter, Manager | "Table {N} - Guest entered wrong OTP {X} times" | HIGH |
| `TABLE_READY_FOR_CLEANING` | Assigned waiter, All waiters | "Table {N} - Ready for cleaning (Session: {duration}, Bill: Rs.{amount})" | MEDIUM |
| `CLEANING_OVERDUE` | Manager, All waiters | "Table {N} - Waiting for cleaning ({X} min)" | HIGH |
| `LONG_STAY_ALERT` | Assigned waiter, Manager | "Table {N} - Occupied for {duration} - Check if assistance needed" | LOW |
| `NEW_ORDER` | Kitchen | "New order #{N} from Table {T}" | HIGH |
| `ORDER_READY` | Assigned waiter | "Order #{N} ready for pickup - Table {T}" | HIGH |
| `BILL_REQUESTED` | Cashier, Assigned waiter | "Table {N} requesting bill" | HIGH |
| `VOICE_MESSAGE` | Assigned waiter | "Voice message from Table {N}" | HIGH |

---

## 12. Settings & Configuration

### 12.1 Configurable Settings UI

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  ⚙️ SETTINGS → TABLE MANAGEMENT                                                │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ═══════════════════════════════════════════════════════════════════════════   │
│  GUEST ASSISTANCE ALERTS                                          [Enabled ✓]  │
│  ═══════════════════════════════════════════════════════════════════════════   │
│                                                                                 │
│  Notify if guest hasn't entered OTP after:      [ 2 ] minutes                  │
│  Notify if guest browsing but hasn't ordered:   [ 5 ] minutes                  │
│                                                                                 │
│  Send notifications to:                                                        │
│  ☑ Assigned waiter                                                             │
│  ☐ All waiters                                                                 │
│  ☐ Manager                                                                     │
│                                                                                 │
│  ═══════════════════════════════════════════════════════════════════════════   │
│  TABLE DURATION COLORS                                                         │
│  ═══════════════════════════════════════════════════════════════════════════   │
│                                                                                 │
│  🟢 Green (Normal):     0 - [ 30 ] minutes                                     │
│  🟡 Yellow (Long):      [ 30 ] - [ 60 ] minutes                                │
│  🟠 Orange (Very Long): [ 60 ] - [ 90 ] minutes                                │
│  🔴 Red (Extended):     [ 90 ]+ minutes                                        │
│                                                                                 │
│  ═══════════════════════════════════════════════════════════════════════════   │
│  LONG STAY ALERTS                                                 [Enabled ✓]  │
│  ═══════════════════════════════════════════════════════════════════════════   │
│                                                                                 │
│  Send first alert after table occupied for:     [ 90 ] minutes                 │
│  Repeat alert every:                            [ 30 ] minutes                 │
│                                                                                 │
│  Send notifications to:                                                        │
│  ☑ Assigned waiter                                                             │
│  ☑ Manager                                                                     │
│                                                                                 │
│  ═══════════════════════════════════════════════════════════════════════════   │
│  TABLE CLEANING                                                   [Enabled ✓]  │
│  ═══════════════════════════════════════════════════════════════════════════   │
│                                                                                 │
│  Alert if table not cleaned within:             [ 10 ] minutes                 │
│                                                                                 │
│  Cleaning checklist:                                              [Enabled ☐]  │
│  (If enabled, staff must confirm checklist items before marking cleaned)       │
│                                                                                 │
│                                        [ Cancel ]    [ Save Settings ]          │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

## Summary of Decisions Implemented

| # | Decision | Implementation |
|---|----------|----------------|
| Q1 | Track QR scan on page load | ✅ POST /api/guest/qr-scan creates QRScanEvent |
| Q2 | Different messages per case | ✅ 3 notification types with specific messages |
| Q3 | Configurable timers | ✅ RestaurantSettings with all timer fields |
| Q4 | Waiter marks "Served" | ✅ OrderItem.served_at + served_by fields |
| Q5 | Bill shows "Served at" only | ✅ Internal view shows order#, customer bill shows served time only |
| Q6 | OTP changes on payment | ✅ endSessionAndChangeOTP() function |
| Q7 | Keep OTP history | ✅ OTPHistory model with full tracking |
| Q8 | Any staff can mark cleaned | ✅ POST /api/tables/:id/mark-cleaned (any staff) |
| Q9 | Cleaning time limit alert | ✅ cleaning_alert_minutes setting + CLEANING_OVERDUE notification |
| Q10 | Color + configurable alert | ✅ Duration colors + long_stay_alert settings |
| Q11 | Standard for waiter, Detailed for manager | ✅ Different dashboard views documented |

---

## Files to Create/Modify

### New Files:
```
/packages/database/prisma/schema.prisma (updates)
/apps/web/lib/services/qr-scan.service.ts
/apps/web/lib/services/session.service.ts (updates)
/apps/web/lib/services/otp.service.ts
/apps/web/lib/services/cleaning.service.ts
/apps/web/lib/services/assistance-timer.service.ts
/apps/web/lib/jobs/assistance-check.job.ts
/apps/web/lib/jobs/long-stay-check.job.ts
/apps/web/lib/jobs/cleaning-alert.job.ts
/apps/web/app/(dashboard)/settings/tables/page.tsx
/apps/web/app/(dashboard)/waiter/components/TableCard.tsx (updates)
/apps/web/app/(dashboard)/manager/components/TableCardDetailed.tsx
/apps/web/app/api/guest/qr-scan/route.ts
/apps/web/app/api/sessions/[id]/end/route.ts
/apps/web/app/api/tables/[id]/mark-cleaned/route.ts
```

---

## End of Session Flow Specification
